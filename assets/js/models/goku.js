class Goku {
  ...
  move() {
    ...
    this.clouds.forEach((cloud) =>
      cloud.move(
        ...
        this.y,
        this.vX,
        this.isRidingCloud.isFliying,
        this.isRidingCloud.leave
      )
    );

    if (!this.movements.right && !this.movements.left && !this.movements.jump) {
  ...
  clear() {
    ...
    this.ondasVital = this.ondasVital.filter(
      (onda) => onda.x < this.ctx.canvas.width
    );
    this.ondasVital = this.ondasVital.filter(
      (onda) => onda.y < this.ctx.canvas.height
    );
    ...
    this.clouds = this.clouds.filter(
      (cloud) => cloud.y < this.ctx.canvas.width
    );
  ...
  animate() {
    ...
    if (
      this.animationTick >= 7 &&
      (this.movements.right || this.movements.left || this.movements.walk)
    ) {
      this.animationTick = 0;
